...
struct TestPrep{TXCA, TYCA, TJCA, TAD}
    M::Int
    N::Int
    X_ca::TXCA
    Y_ca::TYCA
...
function _get_test_prep(M, N, ad_type)
    ...
    return TestPrep(M, N, X_ca, Y_ca, J_ca, ad_backend)
end
...
function doit_in_place(prep::TestPrep)
...
#         println("autosparse_manual, in-place, $ad")
#         p = _get_test_prep(4, 3, ad)
#         @time doit_in_place(p)
#     end
...
function doit_out_of_place(prep::TestPrep)
    # `M` and `N` will be passed via the params argument.
    # M = 4
    # N = 3
    M = prep.M
...
# @testset "out-of-place" verbose=true showtiming=true begin
#     for ad in ["forwarddiff", "reversediff", "zygote"]
#         println("autosparse_manual, out-of-place, $ad")
#         p = _get_test_prep(4, 3, ad)
#         @time doit_out_of_place(p)
#     end
#     # doit_out_of_place(; ad_type="enzymeforward")
#     # doit_out_of_place(; ad_type="enzymereverse")
# end
for ad in ["forwarddiff", "reversediff", "zygote"]
    p = _get_test_prep(4, 3, ad)
