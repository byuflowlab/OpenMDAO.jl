...
function doit_out_of_place_shape_by_conn(; ad_type, sparse_detect_method)
    ...
    X_ca[:a] = 2.0
    X_ca[:b] .= range(3.0, 4.0; length=N)
    # X_ca[:c] .= range(5.0, 6.0; length=M_wrong)
    X_ca[:c] .= 5.0
    ...
    comp = SparseADExplicitComp(ad_backend, f_simple, X_ca; params=params, shape_by_conn_dict=Dict(:c=>true, :d=>true, :f=>true, :g=>true))
