...
function doit_forward_shape_by_conn(; ad_type, disable_prep)
    ...
    comp = MatrixFreeADExplicitComp(ad_backend, f_simple_no_params!, Y_ca, X_ca; disable_prep=disable_prep, shape_by_conn_dict=Dict(:b=>true, :d=>true, :e=>true, :f=>true, :g=>true))
...
function doit_reverse_shape_by_conn(; ad_type, disable_prep)
    ...
    Y_ca = ComponentVector(e=zeros(Float64, N), f=zeros(Float64, M_wrong, N), g=zeros(Float64, N, M_wrong))
    ...
    comp = MatrixFreeADExplicitComp(ad_backend, f_simple_no_params!, Y_ca, X_ca; disable_prep=disable_prep, shape_by_conn_dict=Dict(:c=>true, :d=>true, :f=>true, :g=>true))
