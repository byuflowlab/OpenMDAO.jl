...
function doit_ad(; sparse_detect_method)
    # `M` and `N` will be passed via the params argument.
    N = 3
    M = 4
    params_simple = (M, N)
    ...
    comp = ADSparseExplicitComp(ad, f_simple!, Y_ca, X_ca; params=params_simple)
...
doit_ad(; sparse_detect_method=:iterative)
doit_ad(; sparse_detect_method=:direct)
