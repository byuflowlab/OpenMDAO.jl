...
function Comp2(M, N)
    ...
    X_ca = ComponentVector(a=0.0, b=zeros(Float64, N), c=zeros(Float64, M), d=zeros(Float64, M, N))
    @view(X_ca[:a]) .= 2.0
    @view(X_ca[:b]) .= range(3.0, 4.0; length=N)
    @view(X_ca[:c]) .= range(5.0, 6.0; length=M)
    @view(X_ca[:d]) .= reshape(range(7.0, 8.0; length=M*N), M, N)
    Y_ca = ComponentVector(e=zeros(Float64, N), f=zeros(Float64, M, N), g=zeros(Float64, N, M), h=0.0)
...
get_tags(self::Comp2, varname) = nothing
...
function doit()
    ...
    h_check = 3.0*a.^2
    @test all(outputs_dict["h"] .≈ h_check)
    ...
    @test nnz(getdata(J_ca_sparse)) == N + N + N*M + N*M + M*N + M*N + M*N + M*N + N*M + N*M + 1
    ...
    vals = partials_dict["h", "a"]
    @test size(vals) == (1,)
    dadh_check = 6.0.*a
    @test all(vals .≈ dadh_check)
...
function doit_simple()
    # Create a "simple" sparse component and see if that gives us the same results as the hand-coded one.
    # First need to create a function that does the same thing as Comp2's:
    f_simple! = function (Y, X, params)
        local a = only(X[:a])
        local b = @view X[:b]
        local c = @view X[:c]
        local d = @view X[:d]
    ...
    comp = SimpleAutoSparseForwardDiffExplicitComp(f_simple!, Y_ca, X_ca; params=params_simple)
