...
function doit_simple()
    ...
    comp = SimpleAutoSparseForwardDiffExplicitComp(f_simple!, Y_ca, X_ca; params=params_simple, J_ca=J_ca)
...
function doit_ad()
    ...
    # Now we can create the component.
    ad = ADTypes.AutoSparse(ADTypes.AutoForwardDiff(); sparsity_detector=ADTypes.KnownJacobianSparsityDetector(sparse(getdata(J_ca))), coloring_algorithm=SparseMatrixColorings.GreedyColoringAlgorithm())
