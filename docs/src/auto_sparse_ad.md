...
# Automatic Sparse AD
This section will describe how to create explicit components that are differentiated automatically by Julia AD packages supported by [DifferentiationInterface.jl](https://github.com/JuliaDiff/DifferentiationInterface.jl), with the derivative calculation accelerated by sparse matrix coloring algorithms.
This can yield significant speedups when the Jacobian of the component is sparse.
The sparsity pattern of the Jacobian can be specified by the user or generated by one of the detection algorithms supported by DifferentiationInterface.jl and dependent packages.
...
## The User-Defined Function for Sparse AD
...
Notice that it's identical to the function we used in the dense AD example.
...
Looks OK so far.
But we should check our derivatives, again, just to be safe.
